...
const MaintainFiscalYearPeriods = () => {
  ...
  const [accountingPeriodsData, setAccountingPeriodsData] = useState(fiscalData[2020] || []);
  const [budgetingPeriodsData, setBudgetingPeriodsData] = useState(fiscalData[2020] || []);
  ...
  React.useEffect(() => {
    if (activeTab === 'accountingPeriods') {
      ...
      setAccountingPeriodsData(fiscalData[currentYear] ? JSON.parse(JSON.stringify(fiscalData[currentYear])) : []);
    } else {
      // Ensure a deep copy to prevent direct mutation of fiscalData
      setBudgetingPeriodsData(fiscalData[currentYear] ? JSON.parse(JSON.stringify(fiscalData[currentYear])) : []);
    }
  ...
  const currentAccountingData = useMemo(() => accountingPeriodsData, [accountingPeriodsData]);
  const currentBudgetingData = useMemo(() => budgetingPeriodsData, [budgetingPeriodsData]);
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-gray-900 flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full px-8 bg-white rounded-xl shadow-lg p-8 space-y-6 border border-gray-300">
        ...
        {/* Table Content */}
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-300 overflow-hidden"> {/* Added overflow-hidden */}
          <div className="overflow-x-auto rounded-lg border border-gray-300">
            <table className="min-w-full divide-y divide-gray-300">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {activeTab === 'accountingPeriods' && currentAccountingData.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                      <input
                        type="number"
                        value={row.period}
                        onChange={(e) => handleTableCellChange(row.id, 'period', e.target.value, activeTab)}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                      <input
                        type="number"
                        value={row.subPeriod}
                        onChange={(e) => handleTableCellChange(row.id, 'subPeriod', e.target.value, activeTab)}
                        className="w-20 bg-white border border-gray-300 rounded-md py-1 px-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 text-gray-900"
                      />
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                      <input
                        type="text"
                        value={row.endDate}
                        onChange={(e) => handleTableCellChange(row.id, 'endDate', e.target.value, activeTab)}
                    ...
                    {activeTab === 'accountingPeriods' && (
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                        <select
                          value={row.quarter}
                          onChange={(e) => handleTableCellChange(row.id, 'quarter', e.target.value, activeTab)}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                      <input
                        type="text"
                        value={row.status}
                        onChange={(e) => handleTableCellChange(row.id, 'status', e.target.value, activeTab)}
                ...
                {activeTab === 'budgetingPeriods' && currentBudgetingData.map((row) => (
                  <tr key={row.id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {row.fiscalYear}
                    </td>
