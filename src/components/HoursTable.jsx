...
const ProjectHoursDetails = ({
  ...
  // Updated organization validation - only numeric IDs
  const isValidOrg = (val) => {
    if (!val) return true; // Allow empty
    const trimmed = val.toString().trim();
    return trimmed.length > 0 && /^\d+$/.test(trimmed); // Must be non-empty and numeric
  };
  ...
  useEffect(() => {
    const fetchEmployeesSuggestions = async () => {
      ...
      try {
        ...
        const uniqueOrgIds = [...new Set(suggestions.map(emp => emp.orgId).filter(id => id && /^\d+$/.test(id)))];
        setOrgOptions(uniqueOrgIds);
      } catch (err) {
  ...
  const handlePlcInputChange = (value) => {
    ...
    if (!autoPopulatedPLC) {
      ...
      if (value && !plcOptions.some(opt => opt.value.startsWith(value))) {
        toast.error("No matching Plc for entered value.");
      }
    }
  };
  ...
  const handleAccountChange = (value) => {
    ...
    if (value && !laborAccounts.some(acc => acc.id.startsWith(value))) {
      toast.error("No matching Account for entered value.");
    }
  };
  ...
  const handleOrgChange = (value) => {
    ...
    if (value && !orgOptions.some(o => o.startsWith(value))) {
      toast.error("No matching Organization ID for entered value.");
    }
  };
  ...
  const handleEmployeeDataChange = (empIdx, field, value) => {
    ...
    if (field === 'acctId' && value && !laborAccounts.some(acc => acc.id.startsWith(value))) {
      toast.error("No matching Account for entered value.");
    } else if (field === 'orgId' && value && !orgOptions.some(o => o.startsWith(value))) {
      toast.error("No matching Organization ID for entered value.");
    } else if (field === 'glcPlc' && value && !plcOptions.some(opt => opt.value.startsWith(value))) {
      toast.error("No matching Plc for entered value.");
    }
  };
