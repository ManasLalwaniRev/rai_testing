...
const ProjectPlanForm = ({ projectId, onClose, onPlanCreated, sourcePlan, planType: initialPlanType }) => {
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      try {
        ...
        toast.error('Failed to load templates: ' + (err.response?.data?.message || err.message), { toastId: 'fetch-templates-error' });
        setTemplates([{ id: 1, templateCode: 'SYSTEM', description: 'Default System Template' }]);
        setTemplate('SYSTEM');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!projectId || !versionCode || !planType || !template) {
      toast.error('Missing required parameters for creating plan', { toastId: 'create-plan-error' });
      return;
    }
    ...
    try {
      ...
      toast.error('Error creating plan: ' + (err.response?.data?.message || err.message), { toastId: 'create-plan-error' });
    } finally {
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md border border-gray-300">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-sm font-semibold mb-1">Version Code:</label>
            <input
              type="text"
              value={versionCode}
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              ...
              className={`bg-blue-600 text-white px-3 py-1 rounded text-sm ${
                isSubmitting || isLoadingTemplates ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'
              }`}
            >
