...
const FinancialDashboard = ({ planId, templateId, type }) => {
  ...
  const employeeOptions = useMemo(() => {
    ...
    return ['all', ...Array.from(allEmployees.entries()).map(([id, name]) => ({ id, name }))];
  }, [processedData]);
  ...
  useEffect(() => {
    ...
    if (selectedEmployee !== 'all') {
        filteredPlcs = filteredPlcs.map(plc => ({
            ...plc,
            employees: plc.employees.filter(emp => emp.emplId === selectedEmployee)
        })).filter(plc => plc.employees.length > 0);
    }
    
    // --- Prepare data for Revenue vs. Cost Bar Chart ---
    const totalRevenue = filteredPlcs.reduce((sum, plc) => sum + plc.employees.reduce((s, e) => s + e.revenue, 0), 0);
    const totalBurdenedCost = filteredPlcs.reduce((sum, plc) => sum + plc.employees.reduce((s, e) => s + e.cost, 0), 0);
    const totalRawCost = filteredPlcs.reduce((sum, plc) => sum + plc.employees.reduce((s, e) => s + e.rawCost, 0), 0);
    ...
    <div className="p-6 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 p-4 bg-white rounded-lg shadow">
        <div>
          ...
          >
            {plcOptions.map(plc => (
              <option key={plc} value={plc}>{plc === 'all' ? 'All PLCs' : plc}</option>
            ))}
          </select>
        </div>
        <div>
          <label htmlFor="employee-filter" className="block text-sm font-medium text-gray-700">Filter by Employee</label>
          <select
            id="employee-filter"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 p-4 bg-white rounded-lg shadow">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">Revenue vs. Cost</h2>
          {revenueChartData && <Bar data={revenueChartData} options={{ responsive: true, plugins: { legend: { position: 'top' }} }} />}
        </div>
        <div className="lg:col-span-2 p-4 bg-white rounded-lg shadow">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">Cost Breakdown by PLC</h2>
          {plcPieChartData && <Doughnut data={plcPieChartData} options={{ responsive: true, plugins: { legend: { position: 'top' }} }} />}
        </div>
      </div>
    </div>
  );
