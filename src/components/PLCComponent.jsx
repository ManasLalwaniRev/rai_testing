...
//   const vendorLookupTypeOptions = ["Vendor", "Employee"];
...
//           filteredData.map((item) => ({
//             id: item.projEmplRtKey || item.id,
//             lookupType: item.lookupType || item.type || "Select",
//             empId: item.emplId,
//             employeeName:
//               item.employeeName ||
//               employees.find((emp) => emp.empId === item.emplId)
...
const PLCComponent = ({ selectedProjectId, selectedPlan }) => {
  ...
  const handleUpdateVendor = async (id, updatedData) => {
    ...
    try {
      ...
      await axios.put(`https://test-api-3tmq.onrender.com/ProjVendRt/${id}`, {
        ...
        startDt: new Date(fields.startDate || updatedData.startDate).toISOString(),
        ...
        prev.map((rate) =>
          rate.id === id
            ? {
                ...
                plc: updatedData.plc,
                plcDescription: plcs.find((plc) => plc.laborCategoryCode === updatedData.plc)?.description || updatedData.plcDescription,
                billRate: parseFloat(editVendorBillRate[id] || updatedData.billRate),
                rateType: fields.rateType || updatedData.rateType,
                startDate: fields.startDate || updatedData.startDate,
                endDate: fields.endDate || updatedData.endDate || null,
              }
  ...
  const handleVendorFieldChange = (id, field, value) => {
    ...
    if (field !== "lookupType" && field !== "rateType" && field !== "startDate" && field !== "endDate") {
      setVendorBillingRates((prev) =>
        prev.map((rate) =>
                    ...
                    <td className="border p-2 sm:w-1/5">
                      <div className="flex flex-col sm:flex-row justify-center space-y-1 sm:space-y-0 sm:space-x-2">
                        <button
                          ...
                          className="bg-yellow-500 text-white px-3 py-1 rounded text-xs font-normal hover:bg-yellow-600 transition w-full sm:w-auto"
                          disabled={loading}
                        >
                          Update
                        </button>
                        <button
                          onClick={() => handleDelete(item.id)}
                          className="bg-red-500 text-white px-3 py-1 rounded text-xs font-normal hover:bg-red-600 transition w-full sm:w-auto"
                          disabled={loading}
                        >
                          Delete
                        </button>
