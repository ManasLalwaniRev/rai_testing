...
const PoolRate = ({ userName = 'User' }) => {
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        ...
        const errorMessage = 'Failed to fetch templates: ' + (err.response?.data?.message || err.message || 'Unknown error');
  ...
  useEffect(() => {
    ...
    const fetchPoolsAndRates = async () => {
      ...
      try {
        ...
        const errorMessage = 'Failed to fetch pools or rates: ' + (err.response?.data?.message || err.message || 'Bad request');
        setError(errorMessage);
        console.error(errorMessage, err);
        toast.error(errorMessage, { position: 'top-right', autoClose: 3000 });
      } finally {
  ...
  const handleFindAndReplace = () => {
    ...
    if (selectedPools.length === 0) {
      toast.error('Please select at least one pool to perform find and replace.', {
        position: 'top-right',
        autoClose: 3000,
      });
      return;
      ...
      poolsToProcess.forEach((pool) => {
        ...
        if (currentRate === findValue) {
          ...
          setEditMonths((prev) => {
            ...
            console.log('EditMonths updated after replace:', Array.from(newSet));
            return newSet;
          });
        }
      });
    ...
    const poolScope = ` for pools ${pools
      ...
      .map((p) => p.groupName)
      .join(', ')}`;
    toast.success(`Replaced "${findValue}" with "${replaceValue}" across all months${poolScope}`, {
      position: 'top-right',
      autoClose: 3000,
    });
    console.log(`Find and Replace applied: find=${findValue}, replace=${replaceValue}, pools=${selectedPools}`);
  };
  ...
  const saveMonthlyData = async () => {
    ...
    try {
      const payload = Array.from(editMonths).map((key) => {
        ...
        const targetRate = monthData.rates[poolId].targetRate === '' ? 0 : parseFloat(monthData.rates[poolId].targetRate) || 0;
        return {
      ...
      const updatedMonthlyData = monthlyData.map((item) => {
        ...
        payload.forEach((p) => {
          if (p.month === monthIndex && editMonths.has(`${item.month}_${p.poolId}`)) {
      ...
      setMonthlyData(updatedMonthlyData);
      initialMonthlyData.current = JSON.parse(JSON.stringify(updatedMonthlyData));
      setEditMonths(new Set());
      setError(null);
      ...
      const errorMessage = 'Failed to save monthly data: ' + (err.response?.data?.message || err.message || 'Unknown error');
      setError(errorMessage);
      console.error(errorMessage, err);
  ...
  if (loading && templates.length === 0) {
    return (
      <div className="flex justify-center items-center h-64">
        ...
        <span className="ml-2 text-gray-600 text-sm font-roboto">Loading...</span>
      </div>
    );
  }
      ...
      <div className="p-4 max-w-6xl mx-auto font-roboto">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="lg:w-1/4 space-y-3">
            ...
            <div className="mt-4">
              ...
              >
                ...
                <svg
                  className={`w-5 h-5 transform ${isFindReplaceOpen ? 'rotate-45' : ''} transition-transform-xs`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
              ...
              {isFindReplaceOpen && (
                <div className="mt-3 space-y-3 bg-gray-50 p-4 rounded-lg shadow-sm">
                  <div>
                    <label htmlFor="find" className="block text-xs font-medium text-gray-700 mb-1">
                      Find Value
                    </label>
                    <input
                      id="find"
                  ...
                  <div>
                    <label htmlFor="replace" className="block text-xs font-medium text-gray-700 mb-1">
                      Replace With
                    </label>
                    <input
                      id="replace"
                      ...
                      <div className="flex flex-nowrap overflow-x-auto gap-4 py-2">
                        {pools.map((pool) => (
                          ...
                          >
                            ...
                            />
                            <span className="break-words max-w-[150px]">{pool.groupName}</span>
                          </label>
                        ))}
                      </div>
                    ) : (
                  ...
                  <button
                    ...
                    disabled={loading || isSaving || !findValue || !replaceValue}
                  >
                    Replace All
                  </button>
                </div>
          ...
          <div className="lg:w-3/4">
            {selectedTemplate && selectedYear && (
              <div>
                <h2 className="text-sm font-medium text-gray-900 mb-1">Target Rates</h2>
                      ...
                      {monthlyData.length > 0 && !loading && !error ? (
                        monthlyData.map((data) => (
                          <tr key={data.month} className="bg-white">
                            ...
                            {pools.map((pool) => (
                              <td key={pool.poolId} className="px-2 py-1 text-xs border-b border-gray-200 w-[60px]">
                                <input
                                  type="text"
                                  ...
                                  onChange={(e) => handleTargetRateChange(data.month, pool.poolId, e.target.value)}
                                  disabled={loading || isSaving}
                                />
                              </td>
                            ))}
                        ...
                        <tr>
                          <td colSpan={pools.length + 1} className="px-2 py-4 text-xs text-gray-700 text-center">
                            {loading ? (
