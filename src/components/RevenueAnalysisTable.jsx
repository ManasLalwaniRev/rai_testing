...
      }
      ...
      let employeeAssignment = plcDetail.employees.find(e => e.id === compositeKey);
      ...
      if (!employeeAssignment) {
        ...
        const summaryBurdenedCost = summaryRawCost + summaryFringe + summaryOverhead + summaryGNA;
        ...
        employeeAssignment.profit = employeeAssignment.revenue - employeeAssignment.cost;
        employeeAssignment.profitOnCost = employeeAssignment.cost !== 0 ? `${((employeeAssignment.profit / employeeAssignment.cost) * 100).toFixed(2)}%` : '0.00%';
        employeeAssignment.profitOnRevenue = employeeAssignment.revenue !== 0 ? `${((employeeAssignment.profit / employeeAssignment.revenue) * 100).toFixed(2)}%` : '0.00%';
...
    });
    ...
    totalLaborCostRaw = Array.from(plcMap.values()).reduce((sum, plc) => sum + plc.rawCost, 0);
    totalBurdenedLaborCost = Array.from(plcMap.values()).reduce((sum, plc) => sum + plc.burdenedCost, 0);
    totalLaborRevenue = apiResponse.employeeForecastSummary.reduce((sum, emp) => sum + (emp.revenue || 0), 0);
...
            id: `nonlabor-directcost-${forecast.year}-${forecast.month}-${directCostSummary.emplId}-${forecast.dctId}-${Date.now() + Math.random()}`,
...
  const finalTotalRevenueOverall = apiResponse.totalRevenue || totalLaborRevenue;
...
  const [atRiskFundedAmount, setAtRiskFundedAmount] = useState("$0.00 - Placeholder");
...
      <div className={`overflow-x-auto ${getGlassmorphismClasses()}`}>
...
              <tr>
                <td colSpan={8} className="py-8 text-center text-gray-600 text-lg">
...
                                  <tr
                                    className="bg-gray-100 bg-opacity-30 hover:bg-gray-100 hover:bg-opacity-60 text-xs group"
                                  >
...
                            <td className="py-2 px-4 text-right whitespace-nowrap text-gray-800" colSpan="2"></td>
                          </tr>
...
                                <td className="py-2 pl-16 pr-4 text-left text-sm font-semibold text-gray-700 uppercase" colSpan="2">Employee ID - Period</td>
...
                                <td className="py-2 px-4 text-right text-sm font-semibold text-gray-700 uppercase" colSpan="2"></td>
                              </tr>
