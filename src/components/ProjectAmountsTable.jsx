...
const ProjectAmountsTable = ({
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [startDate, endDate, planId, refreshKey, propFiscalYear]); // Added propFiscalYear to refetch on fiscal year change
  ...
  const handleForecastAmountBlur = async (empIdx, uniqueKey, value) => {
    ...
    try {
      ...
      setEmployees((prev) => {
        ...
        if (forecastIndex >= 0) {
          updatedForecasts[forecastIndex] = {
            ...updatedForecasts[forecastIndex],
            ...(planType === "EAC"
              ? { actualamt: payload.actualamt }
              : { forecastedamt: payload.forecastedamt }),
          };
        } else {
          // If new forecast, add it
          updatedForecasts.push({
            ...payload,
            value: planType === "EAC" ? payload.actualamt : payload.forecastedamt,
          });
        }
        updatedEmp.emple = {
          ...updatedEmp.emple,
      ...
      // Trigger refetch from server to ensure persistence
      if (onSaveSuccess) {
        onSaveSuccess();
      }
    } catch (err) {
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md text-sm">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                value={startDate}
                readOnly
                className="w-full border border-gray-300 rounded-md p-2 text-xs bgb-gray-100 cursor-not-allowed"
              />
            </div>
            <div className="mb-4">
              <label className="block text-gray-700 text-xs font-medium mb-1">
